...
export async function POST(req: Request) {
  ...
  const headersRecord: Record<string, string> = Object.fromEntries(req.headers.entries());

  try {
    // Verify the webhook using the DodoPayments SDK
    const event = await dodopayments.webhooks.unwrap(body, { headers: headersRecord });

...
import { Globe, BarChart3, CheckCircle2, TrendingUp, Zap, Shield } from "lucide-react";
...
function NodeNetwork() {
  const nodes = [
    { x: 20, y: 30 }, { x: 50, y: 15 }, { x: 80, y: 40 },
    { x: 35, y: 65 }, { x: 65, y: 70 }, { x: 85, y: 80 },
  ];
...
export function BentoGrid() {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={card} whileHover={{ y: -4 }} transition={{ duration: 0.2 }}
        ...
        <div>
          ...
          <p className="text-muted-foreground text-base max-w-md leading-relaxed">
            Analyzing{" "}<TypedDomain />{" "}— title tags, viewport, robots meta, open graph, alt texts, and more.
          </p>
        </div>
        <div className="flex gap-2 flex-wrap mt-auto">
          {["Meta Tags", "OG Data", "Alt Texts", "Robots", "Viewport"].map((t) => (
            <span key={t} className="text-xs bg-primary/10 text-primary border border-primary/20 rounded-full px-3 py-1 font-medium">{t}</span>
          ))}
        </div>
      </motion.div>
      ...
      <motion.div variants={card} whileHover={{ y: -4 }} transition={{ duration: 0.2 }}
        ...
        <p className="text-muted-foreground text-base mb-6">Proprietary SEO score out of 100 — know exactly where you stand.</p>
        ...
        <div className="mt-3 h-2 rounded-full bg-primary/10 overflow-hidden">
          <motion.div className="h-full bg-gradient-to-r from-primary/70 to-primary rounded-full"
            initial={{ width: "0%" }}
            animate={{ width: "87%" }}
      ...
      <motion.div variants={card} whileHover={{ y: -4 }} transition={{ duration: 0.2 }}
        ...
        <p className="text-muted-foreground text-base mb-6">Plain-English fixes, not just a list of problems.</p>
        <div className="mt-auto space-y-2">
          {[
            ...
            <motion.div key={fix.text}
              className="flex items-center gap-2.5 text-sm"
              initial={{ opacity: 0, x: -8 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.8 + i * 0.15, duration: 0.4 }}>
              <CheckCircle2 className={`h-4 w-4 shrink-0 ${fix.done ? "text-primary" : "text-muted-foreground/40"}`} />
              <span className={fix.done ? "line-through text-muted-foreground/60" : "text-foreground"}>{fix.text}</span>
            </motion.div>
          ))}
        </div>
      </motion.div>
      ...
      <motion.div variants={card} whileHover={{ y: -4 }} transition={{ duration: 0.2 }}
        ...
        <div>
          ...
          <p className="text-muted-foreground text-base max-w-md leading-relaxed">
            Verify content volume, keyword structure, and internal linking to prevent thin-content penalties.
          </p>
        </div>
        <div className="grid grid-cols-3 gap-4 mt-8">
          {[
            ...
            <div key={s.label} className="rounded-xl bg-primary/8 border border-primary/15 p-3 text-center">

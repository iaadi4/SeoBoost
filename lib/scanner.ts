...
export async function scanDomain(url: string): Promise<SEOReport> {
  try {
    ...
    return {
      ...
      title: {
        ...
        message: titlePass
          ? 'Optimal length'
          : title
            ? `Title length (${title.length}) should be 30-65 chars`
            : 'Missing Title tag',
      },
      description: {
        value: description,
        ...
        message: descPass
          ? 'Optimal length'
          : description
            ? `Description length (${description.length}) should be 70-160 chars`
            : 'Missing Meta Description',
      },
      h1: {
        count: h1Count,
        value:
          h1s.first().text().trim().substring(0, 100) +
          (h1Count > 0 && h1s.first().text().length > 100 ? '...' : ''),
        pass: h1Pass,
        message: h1Pass
          ? 'Only one H1 tag found (Good)'
          : h1Count === 0
            ? 'Missing H1 tag'
            : 'Multiple H1 tags found. Use exactly one.',
      },
      images: {
        total: images.length,
        missingAlt,
        pass: imagesPass,
        message: imagesPass
          ? 'Images formats optimized'
          : `${missingAlt} out of ${images.length} images are missing alt text.`,
      },
      links: {
        total: links.length,
        internal,
      ...
      content: {
        ...
        message: wordPass
          ...
          : `Thin content detected (${wordCount} words). Aim for at least 300 words.`,
      },
      technical: {
        hasViewport,
        hasFavicon,
    ...
    throw new Error(
      'Failed to scan the domain. Make sure the URL is correct and accessible.'
    )
  }
}
